...
const ProductItem = ({ item, index, marginBottom, moreType, hideAddToCart }) => {
  ...
  const productUrl = `/product?id=${productSlug}&idx=${index}`;

  return (
    <>
      <div className={`sb-menu-item sb-mb-${marginBottom}`}>
        <Link href={productUrl} className="sb-cover-frame">
            <img src={item.image} alt={item.title} />
            <div dangerouslySetInnerHTML={{__html : item.badge}} />
        </Link>
        <div className="sb-card-tp">
            <h4 className="sb-card-title"><Link href={productUrl}>{item.title}</Link></h4>
        ...
        <div className="sb-card-buttons-frame">
          {/* button */}
          {moreType != 2 ? (
          <Link href={productUrl} className="sb-btn sb-btn-2 sb-btn-gray sb-btn-icon sb-m-0">
            <span className="sb-icon">
              <img src="/img/ui/icons/arrow.svg" alt="icon" />
            </span>
          </Link>
          ) : (
          <Link href={productUrl} className="sb-btn sb-btn-gray">
            <span className="sb-icon">
              <img src="/img/ui/icons/arrow.svg" alt="icon" />
            </span>
            <span>Details</span>
